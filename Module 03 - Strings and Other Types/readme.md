...
```python
msg = '     Welcome to SAINTCON 2019!!!!     '

left = msg.lstrip()         # 'Welcome to SAINTCON 2019!!!!     '
right = msg.rstrip()        # '     Welcome to SAINTCON 2019!!!!'
full = msg.strip()          # 'Welcome to SAINTCON 2019!!!!'
```
...
# Tuple
Tuples are similar to list except where a list can be modified a tuple is read only. Creation of a list is done with parentheses.
...
Tuples behave essentially the same as a list unless you try to modify one of the elements.  The following snippet of code will throw a TypeError.
...
You may ask if it is possible to crete a for loop that executes 10 ties without having to manage the expression that breaks the loop.  
...
Removing items from a dictionary can be done with either `remove()` or `discard()`.  I recommend using `discard()` as `remove()` will throw a TypeError if an item that is not in the set is removed.
...
print(count)
# will print - {'c': 3, 'd': 1, 'g': 1, 'f': 1, 'l': 3, 'p': 2, 's': 3, 'r': 2, 't': 1, 'x': 1}
```

However, if you saw the comment you notice that I said that this is the hard way to get counts like this.  Now for a slight segue to show the easy way and introduce some specialized collections.  These high performance collections actually provide two collections that could generate this data easier.  The `defaultdict` and `Counter`.  `defaultdict` allows you to specify a value for when the key does not exist in the collection and `Counter` is optimized for counting hashable objects.  
...
print(count)
# will print defaultdict(<type 'int'>, {'a': 3, 'c': 3, 'e': 2, 'd': 1, 'g': 1, 'f': 1, 'i': 7, 'l': 3, 'o': 2, 'p': 2, 's': 3, 'r': 2, 'u': 2, 't': 1, 'x': 1})

from collections import Counter
count = Counter(text)

print(count)
# will print Counter({'i': 7, 'a': 3, 'c': 3, 'l': 3, 's': 3, 'e': 2, 'o': 2, 'p': 2, 'r': 2, 'u': 2, 'd': 1, 'g': 1, 'f': 1, 't': 1, 'x': 1}) 
```

As you can see the data is very similar.  The only difference is the order that the items are shown.  If the order matters, and some times it does, the `orderdict` can solve this problem.  
